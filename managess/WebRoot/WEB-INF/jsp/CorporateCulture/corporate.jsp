...
<div class="nav" id="container">
	<div class="main_top">
		...
		<ul class="main_top_title_r">
			...
			<li class="main_top_title_r_a" onclick="showOpen_ajax(document.getElementById('container'),'添 加','corporate.htm?action=addpro&obj=',610,480);">添 加</li>
...
function getprotype(page,_index,_len){
	...
	var url = "corporate.htm?action=protypeSearch";
   ...
   if(len>0){
	   ...
	   //	var cell6 =row.insertCell();
		...
		//cell7.innerHTML ='<!--<span onclick="showOpen_ajax(container,\'修改栏目内容\',\'colcon.htm?action=addpro&obj='+info[i][0]+'&pro='+info[i][7]+'\',610,480);">修 改</span--><span onclick="delconfirm(\''+info[i][0]+'\');">删 除</span>';
   		cell7.innerHTML ='<span onclick="showOpen_ajax(container,\'修 改\',\'corporate.htm?action=addpro&obj='+info[i][0]+'&pro='+info[i][7]+'\',610,480);">修 改</span><span onclick="delconfirm(\''+info[i][0]+'\');">删 除</span>';
...
function deleteproType(info){
	var pars ="flag=deleteproType&id="+info;
	    	var url = "corporate.htm?action=deleteproType";
	    	var reqAjax = new Ajax.Request(    
	        	url,    
	        	{    
	            	method: 'post', 
