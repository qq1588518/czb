...
<BODY>
<form id="theForm" name='theForm' method='post' action="pronews.htm?action=saveProtype" encType="multipart/form-data">
<div class="pwnav">
	<ul class="pwcon">
		<li >
		...
		<li style="display:none">bakcground-color:red;f<br /><br /></li>
		<!--li>
...
function test22(){
    if(document.getElementById("modify_content").getElementsByTagName("textarea").length>0){
		...
		for(var i=0;i<len;i++){
			document.getElementById("tjym").insertAdjacentHTML('BeforeEnd',"<span id='num"+view_index+"' style='margin-right:8px;cursor:pointer'  onclick='changecontent("+view_index+");'>"+view_index+"</span>");
...
function add(){
	...
	if(view_index<5){
		...
		if(value.search(/[^&nbsp;*]/gi)!=-1 && !isEdit){
			...
			document.getElementById("tjym").firstChildinsertAdjacentHTML('BeforeEnd',"<span id='num"+view_index+"' style='margin-right:8px;cursor:pointer'  onclick='changecontent("+view_index+");'>"+view_index+"</span>");
