...
<div class="nav" id="container">
	<div class="main_top">
		...
		<ul class="main_top_title_r">
			...
			<li class="main_top_title_r_a" onclick="showOpen_ajax(document.getElementById('container'),'添加类别','answertype.htm?action=addType&obj=',400,270);">添 加</li>
...
function getprotype(page,_index,_len){
	...
	var url = "answertype.htm?action=typeSearch";
...
function getprotypeResponse(response){
   ...
   if(len>0){
	   ...
	   //	var cell6 =row.insertCell();
	   	...
	   	if(info[i][2]==0){
	   		cell7.innerHTML ='<span onclick="showOpen_ajax(container,\'修改类别\',\'answertype.htm?action=modifyType&obj='+encodeURIComponent(info[i])+'\',400,270);">修 改</span><span onclick="delconfirm(\''+info[i][0]+'\');">删 除</span';   
		}else{
			cell7.innerHTML ='<span onclick="showOpen_ajax(container,\'修改类别\',\'answertype.htm?action=modifyType&obj='+encodeURIComponent(info[i])+'\',400,270);">修 改</span><span onclick="delconfirm(\''+info[i][0]+'\');">删 除</span>';   
		}
   	}
...
function deleteproType(info){
	...
	var pars ="flag=deleteType&fbid="+proid;
	    	var url = "answertype.htm?action=deleteType";
	    	var reqAjax = new Ajax.Request(    
	        	url,    
	        	{    
	            	method: 'post', 
