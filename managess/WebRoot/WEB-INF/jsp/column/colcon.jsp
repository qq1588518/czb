...
<html>
		...
		<div class="nav" id="container">
			<div class="main_top">
				...
				<ul class="main_top_title_r">
					...
					<li class="main_top_title_r_a" onclick="showOpen_ajax(document.getElementById('container'),'栏目内容添加','colcon.htm?action=addpro&obj=',610,480);">
						添 加
					</li>
			...
			<table class="main_select" cellpadding="0" cellspacing="0" align=center>
				<tr align=left>
					<td><span>关键字：</span>
					<input type="text" class="text" id="title" onblur="getprotype();">
					</td>
					<td><span>栏目类别：</span>
					<select name="sub_protype" size="1" style="width:120px;height:20px;margin-right:20px;" class="Tpcform-dorpdownlist" id="sub_protype"  onchange="getprotype();">
	...
	<script>
		...
		function getprotype(page, _index, _len) {
			...
			var pars = "flag=getprotype&title=" + title + "&sub_protype=" + sub_protype + "&index=" + index + "&length=" + item;
			var url = "colcon.htm?action=protypeSearch";
		...
		function getprotypeResponse(response) {
			...
			if(len > 0) {
				for(var i = 0; i < len; i++) {
					...
					cell4.innerText = info[i][10];
					;
					var cell5 = row.insertCell();
					cell5.innerText = info[i][9];
		...
		function deleteproType(info) {
			...
			var url = "colcon.htm?action=deleteproType";
			var reqAjax = new Ajax.Request(url, {
				method : 'post',
				parameters : pars,
				onCreate : loadingFun,
