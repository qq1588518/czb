...
<BODY>
<form id="theForm" name='theForm' method='post' action="colcon.htm?action=saveProtype" encType="multipart/form-data">
<div class="pwnav">
	<ul class="pwcon">
		<li >
		...
		<li>
			<span>标题图片：</span><input class="file"  id="read_pic"  type="file" name="picproduct" style="width:315px;"/><input id="pic" name="pic_addr" type="hidden" value="${pic}"/></li>
		<li >
			<span>所属栏目：</span><select onchange ="search_sub_pro(this);" name="select_protype" size="1" style="width:120px;height:20px;" class="Tpcform-dorpdownlist" id="select_protype"  >
			<option value="0">--请选择--</option>
			<c:forEach var="clime" items="${protype_clime }">
				<option value="${clime.proId }" <c:if test="${pro==clime.proId }">selected="selected"</c:if>>${clime.proName }</option>
			</c:forEach>
			</select>
			<select name="sub_protype" size="1" style="width:120px;height:20px;margin-right:20px;" class="Tpcform-dorpdownlist" id="sub_protype"  >
			<option value="0">--请选择--</option>
			<c:forEach var="clime" items="${sub_pro }">
				<option value="${clime.proId }" <c:if test="${subPro==clime.proId }">selected="selected"</c:if>>${clime.proName }</option>
			</c:forEach>
			</select>
			...
			<input type="hidden" id="conpicimg" name="conpicimg" value="${conpicimg}"/>
		</li>
		
		<!-- 添加文件图片上传 -->
		<input type="hidden" id="flagfile" name="flagfile" />
		<li>内容图片上传：<span id="uploadDiv"><input type="file" name="file1" id="file1" onchange="ff()"/> </span>
    <table id="table1" border="0" cellpadding="0" cellspacing="0"></table>
    <input type="button" value="继续上传" onclick="addFile();"/>
...
function del(){
	...
	//alert(document.getElementById("tjym").firstChild.outerHTML);
	view_index--;
	editor.setHTML("",true);
}
...
function saveProduct(){
	...
	var column_type = document.getElementById("sub_protype").value;
 ...
 var n=0;
		function addFile(){ 
			if(table1.rows.length<5){
				...
				a=oTR.insertCell();
				oTD=oTR.insertCell();
				oTD.innerHTML+="<input type='file' name='file"+n+"' size='40'><input type='button' onclick='deleteFile("+tmpNum+")' value='删除'>";
				n++;
			}
		}
	...
	function ff(){	document.getElementById("flagfile").value=document.getElementById("file1").value;}	
