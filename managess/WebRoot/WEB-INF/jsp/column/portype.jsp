...
<div class="nav" id="container">
	<div class="main_top">
		...
		<ul class="main_top_title_r">
			...
			<li class="main_top_title_r_a" onclick="showOpen_ajax(document.getElementById('container'),'栏目添加','protype.htm?action=addpro&obj=null',400,300);">添 加</li>
...
function getprotype(id){
	...
	var url = "protype.htm?action=protypeSearch";
...
function getprotypeResponse(response){
   ...
   if(len>0){
	   ...
	   //	cell1.innerHTML ='<img src="images/index/detail_have.jpg" onclick="showdetail(this,document.getElementById(\'item1\'));"/>'; 
	   	...
	   	cell4.innerHTML = '<img src="'+info[i][4]+'" style="height:30px;width:40px;"/>';
	   	...
	   	if(info[i][2]==0){
			...
			cell7.innerHTML ='<!--span onclick="getprotype();">返  回</span--><span onclick="showOpen_ajax(container,\'修改栏目\',\'protype.htm?action=addpro&obj='+encodeURIComponent(info[i])+'\',400,300);">修 改</span><span onclick="delconfirm(\''+info[i][0]+'\');">删 除</span>';   
		}
   	}
   }else{
   var row = protype_obj.insertRow();
...
function delconfirm(info){
	var a = confirm("确定删除栏目吗？删除后所有的栏目内容全部都删除");
	var proid = info;
	if(a){
		deleteproType(proid);
	}else{
...
function deleteproType(info){
	...
	var pars ="flag=deleteproType&id="+proid;
	    	var url = "protype.htm?action=deleteproType";
	    	var reqAjax = new Ajax.Request(    
	        	url,    
	        	{    
	            	method: 'post', 
