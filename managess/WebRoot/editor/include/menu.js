...
var sMenu1="<TABLE border=0 cellpadding=0 cellspacing=0 class=Menu width=150><tr><td width=18 valign=bottom align=center style=''><\/td><td width=132 class=RightBg><TABLE border=0 cellpadding=0 cellspacing=0>";
...
function showContextMenu(event){
	...
	if (isControlSelected("IMG")){
		sMenu += getMenuRow("", "ShowDialog('dialog/img.jspa', 350, 315, true)", "img.gif", "图片属性...");
		sMenu += sMenuHr;
...
function showToolMenu(menu){
	...
	case "object":		// 对象效果菜单
		...
		sMenu += getMenuRow("", "ShowDialog('dialog/backimage.jsp', 350, 210, true)", "bgpic.gif", "背景图片");
		sMenu += sMenuHr;
		...
		break;
	case "component":	// 组件菜单
		sMenu += getMenuRow("", "ShowDialog('dialog/img.jspa', 350, 315, true)", "img.gif", "插入或修改图片");
