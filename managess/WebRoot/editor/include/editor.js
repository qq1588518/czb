...
document.onreadystatechange = function() {
	...
	if (!config.License){
		try{
			eWebEditor_License.innerHTML = "&copy; <a href='http://www.goldingsoft.com' target='_blank'><font color=#000000>goldingsoft.com</font></a>";
		}
		catch(e){
		}
	}
...
// 显示帮助
function onHelp(){
	ShowDialog('dialog/help.html','400','300');
	return false;
}
...
// 在当前文档位置插入.
function insertHTML(html,imgurl,id) {
	...
	if(id!=undefined && imgurl!=undefined){
		eWebEditor.document.getElementById(id).src=imgurl;
	}
}

// 设置编辑器的内容
function setHTML(html,flag) {
	ContentEdit.value = html;
	switch (sCurrMode){
	case "CODE":
		...
		eWebEditor.document.body.contentEditable=flag?"true":"false";
		eWebEditor.document.body.unselectable="on";
		eWebEditor.document.body.onselectstart=function(){return false};
		eWebEditor.document.body.onkeydown=function(){if(eWebEditor.event.keyCode==8) eWebEditor.event.returnValue=false;};
...
function Maximize() {
	if (!validateMode()) return;
	window.open("dialog/fullscreen.html?style="+config.StyleName, 'FullScreen'+sLinkFieldName, 'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,fullscreen=yes');
}
...
function createLink(){
	...
	ShowDialog("dialog/hyperlink.html", 350, 170, true);
}
...
// 拼写检查
function spellCheck(){
	ShowDialog('dialog/spellcheck.html', 300, 220, true)
}

// 查找替换
function findReplace(){
	ShowDialog('dialog/findreplace.html', 320, 165, true)
}
...
function mapEdit(){
	...
	window.open("dialog/map.html", 'mapEdit'+sLinkFieldName, 'toolbar=no,location=no,directories=no,status=not,menubar=no,scrollbars=no,resizable=yes,width=450,height=300');
}
