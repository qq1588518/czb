package com.ztenc.oa.proj.web.pronews;
...
public class ProNewsController extends MultiActionController {
	
	private ProNewsService proNewsService;
	public void setProNewsService(ProNewsService proNewsService) {
		this.proNewsService = proNewsService;
	}
String result = "";
	...
	public ModelAndView getUrl(HttpServletRequest request,
		...
		return new ModelAndView("Pronews/pronews",model);
	}
	...
	public ModelAndView addpro(HttpServletRequest request,
		...
		if(!obj.equals("")){
			List corporate = proNewsService.getCorporate(obj);
			String index = "";
			...
			for(Iterator iter = corporate.iterator();iter.hasNext();){
				ProNews proNews = (ProNews) iter.next();
				model.put("id", proNews.getId());
				model.put("title", proNews.getTitle());
				model.put("outurl", proNews.getOutUri());
				model.put("dates", proNews.getDates());
				String urls = proNews.getConUri();
				String[] url = urls.split(",");
				int len = url.length;
				
				for(int i=0;i<len-1;i++){
					
					String content = proNewsService.getContentFromFile(saveDirectory, url[i]);
		...
		return new ModelAndView("Pronews/addpronews",model);
	}
	...
	public ModelAndView saveProtype(HttpServletRequest request,
		...
		response.setContentType("text/html;charset=utf-8");
		request.setCharacterEncoding("utf-8");
		System.out.println("4444444 4443 33333333333");
		    File dir2;
		  //modify by Tim 20110913 ssczb -> managess
		    ...
		    java.util.Date p_date = p_cal.getTime();
				...
				String filepath = "upload/ProNews/file/";
		        ...
		        if (part.isParam()) {  //判断是不是“文本域”控件，如果不是“文本域”，则执行这句话。
		            ...
		            if(name.equals("name_product")){
		            	...
		            	if(id.equals("")){
			            	...
			            	for(int y=0;y<len;y++){
			            		url += proNewsService.saveContentToFile(title,arr.getString(y),saveDerectory3,filenames,y,total,total_filenames,filepath,dates)+",";
			            	}
		            	}else{
		            		List addr = proNewsService.getCorporate(id);
		            		String con_arr = ((ProNews)addr.toArray()[0]).getConUri();
		            		...
		            		for(int i=0;i<len;i++){
		            			...
		            			if(i!=0 && i==len-1){
		            				...
		            				proNewsService.deleteFromFile(saveDerectory3,total_filenames);
		            			}else{
		            				...
		            				proNewsService.deleteFromFile(saveDerectory3,filenames);
		            			}
		            		}
		            		...
		            		if(len>tmp_len){
		            			...
		            			for(int k=0;k<len;k++){
			            			url += proNewsService.saveContentToFile(title,arr.getString(k),saveDerectory3,real_filename,k,total,total_filenames,filepath,dates)+",";
			            		}
		            		}else if(tmp_len>len){
		            			...
		            			for(int k=0;k<len;k++){
			            			url += proNewsService.saveContentToFile(title,arr.getString(k),saveDerectory3,tmp_filename,k,total,total_filenames,filepath,dates)+",";
			            		}
		            		}
...
//		            }
		   ...
		   if(id.equals("")){//如果proId值为空，则为新增，
				JSONObject obj=proNewsService.saveProduct(title,url,outurl,datetime1);
				...
				JSONObject obj=proNewsService.alterProduct(title,url,outurl,datetime1,id);
				if(!obj.getBoolean("flag")){
...
//		return new ModelAndView("column/portype",model);
	...
	public ModelAndView protypeSearch(HttpServletRequest request,
		...
		result = proNewsService.getprotype(title,index,length);
	...
	public ModelAndView deleteproType(HttpServletRequest request,
		...
		List addr = proNewsService.getCorporate(id);
		result = proNewsService.deleteproType(id);
		String con_arr = ((ProNews)addr.toArray()[0]).getConUri();
		String[] con_addr = con_arr.split(",");
		String total_filenames = "";
		String filenames = "";
