package com.ztenc.oa.proj.web.proanswertype;
...
import com.ztenc.oa.proj.service.proanswertype.AnswerTypeService;
...
public class AnswerTypeController extends MultiActionController {
	...
	public ModelAndView getUrl(HttpServletRequest request,
		...
		return new ModelAndView("proanswertype/answertype",model);
	}
	...
	public ModelAndView deleteType(HttpServletRequest request,
		...
		result = answerTypeService.deleteType(info);
	...
	public ModelAndView modifyType(HttpServletRequest request,
		...
		return new ModelAndView("proanswertype/addType",model);
	}
...
//				String datetime1 = tempDate1.format(p_date);
		   ...
		   if(proId.equals("")){//如果proId值为空，则为新增，
			   ...
			   System.out.println(proid);
				JSONObject obj=answerTypeService.saveType(proName,remark,proAddress,level);
			   ...
			   if(proAddress.equals("")){
				   proAddress = pic;
				 //ReadServiceImpl ie=new ReadServiceImpl();
					JSONObject obj=answerTypeService.alterType(proName,remark,proAddress,proId,level);
					if(!obj.getBoolean("flag")){
	...
	public ModelAndView typeSearch(HttpServletRequest request,
		...
		result = answerTypeService.getType(index, length);
		response.getWriter().print(result);
		response.getWriter().close();
		return null;
	}
