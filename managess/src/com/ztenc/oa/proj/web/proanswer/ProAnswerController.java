package com.ztenc.oa.proj.web.proanswer;
...
import com.ztenc.oa.proj.service.proanswer.ProAnswerService;
import com.ztenc.oa.proj.service.pronews.ProNewsService;
...
public class ProAnswerController extends MultiActionController {
	
	private ProAnswerService proAnswerService;
	public void setProAnswerService(ProAnswerService proAnswerService) {
		this.proAnswerService = proAnswerService;
	}
String result = "";
	...
	public ModelAndView getUrl(HttpServletRequest request,
		...
		return new ModelAndView("Proanswer/proanswer",model);
	}
	...
	public ModelAndView addpro(HttpServletRequest request,
		...
		System.out.println("pro=="+request.getParameter("pro"));
		String pro = new String(request.getParameter("pro").getBytes("iso8859-1"),"utf-8");
		if(!obj.equals("")){
			//modify by Tim 20110908
//			System.out.println("pro:"+pro);
//			List list = proAnswerService.getTypeName(pro);
//			for(Iterator iter = list.iterator();iter.hasNext();){
//				AnswerType answerType =(AnswerType) iter.next();
//				System.out.println("answerType.getTypeno():"+answerType.getTypeno());
//				model.put("typeno", answerType.getTypeno());
//			}

			List corporate = proAnswerService.getCorporate(obj);
			String index = "";
			...
			for(Iterator iter = corporate.iterator();iter.hasNext();){
				ProAnswer proAnswer = (ProAnswer) iter.next();
				model.put("id", proAnswer.getId());
				model.put("title", proAnswer.getTitle());
				model.put("outurl", proAnswer.getOutUri());
				model.put("dates", proAnswer.getDates());
				String urls = proAnswer.getConUri();
				String[] url = urls.split(",");
				int len = url.length;
				
				for(int i=0;i<len-1;i++){
					
					String content = proAnswerService.getContentFromFile(saveDirectory, url[i]);
		...
		List list = proAnswerService.getTypes();
		model.put("types", list);
		return new ModelAndView("Proanswer/addproanswer",model);
	}
		...
		request.setCharacterEncoding("utf-8");
		  ...
		  //modify by Tim 20110913 ssczb -> managess
		    ...
		    java.util.Date p_date = p_cal.getTime();
				...
				String filepath = "upload/ProAnswer/file/";
		        ...
		        if (part.isParam()) {  //判断是不是“文本域”控件，如果不是“文本域”，则执行这句话。
		            ...
		            if(name.equals("name_product")){
		            	...
		            	if(id.equals("")){
			            	...
			            	for(int y=0;y<len;y++){
			            		url += proAnswerService.saveContentToFile(title,arr.getString(y),saveDerectory3,filenames,y,total,total_filenames,filepath,dates)+",";
			            	}
		            	}else{
		            		List addr = proAnswerService.getCorporate(id);
		            		String con_arr = ((ProAnswer)addr.toArray()[0]).getConUri();
		            		...
		            		for(int i=0;i<len;i++){
		            			...
		            			if(i!=0 && i==len-1){
		            				...
		            				proAnswerService.deleteFromFile(saveDerectory3,total_filenames);
		            			}else{
		            				...
		            				proAnswerService.deleteFromFile(saveDerectory3,filenames);
		            			}
		            		}
		            		...
		            		if(len>tmp_len){
		            			...
		            			for(int k=0;k<len;k++){
			            			url += proAnswerService.saveContentToFile(title,arr.getString(k),saveDerectory3,real_filename,k,total,total_filenames,filepath,dates)+",";
			            		}
		            		}else if(tmp_len>len){
		            			...
		            			for(int k=0;k<len;k++){
			            			url += proAnswerService.saveContentToFile(title,arr.getString(k),saveDerectory3,tmp_filename,k,total,total_filenames,filepath,dates)+",";
			            		}
		            		}
...
//		            }
		   ...
		   if(id.equals("")){//如果proId值为空，则为新增，
				JSONObject obj=proAnswerService.saveProduct(title,url,outurl,datetime1,typeno);
				...
				JSONObject obj=proAnswerService.alterProduct(title,url,outurl,datetime1,typeno,id);
				if(!obj.getBoolean("flag")){
...
//		return new ModelAndView("column/portype",model);
	...
	public ModelAndView protypeSearch(HttpServletRequest request,
		...
		result = proAnswerService.getprotype(title,index,length);
	...
	public ModelAndView deleteproType(HttpServletRequest request,
		...
		List addr = proAnswerService.getCorporate(id);
		result = proAnswerService.deleteproType(id);
		String con_arr = ((ProAnswer)addr.toArray()[0]).getConUri();
		String[] con_addr = con_arr.split(",");
		String total_filenames = "";
		String filenames = "";
