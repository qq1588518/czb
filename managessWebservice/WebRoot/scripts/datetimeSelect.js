...
//------------------ 样式定义 ---------------------------//
var s_tiannet_body_width = 150;//控件宽
var s_tiannet_body_height = 191;//控件高
//功能按钮样式
...
//日期选择控件体的样式
var s_tiannet_body = "width:"+s_tiannet_body_width+";height:"+s_tiannet_body_height+";background-color:#2650A6;display:none;z-index:9998;position:absolute;" +
...
function tiannetPopCalendar(){
	...
	var objCalendar = tGetElementById("divTiannetDate");
	...
	if (iY <0)		
		iY=0;
	*/
	var e = tiannetOutObject;
	var t = e.offsetTop,  h = e.clientHeight, l = e.offsetLeft, p = e.type;
	while (e = e.offsetParent){t += e.offsetTop; l += e.offsetLeft;}
    
    var cw = objCalendar.clientWidth, ch = objCalendar.clientHeight;
	cw = s_tiannet_body_width;
	ch = s_tiannet_body_height;//第一次总是不能得到clientHeight，郁闷
    var dw = document.body.clientWidth, dl = document.body.scrollLeft, dt = document.body.scrollTop;
    
    //alert("控件高：" + ch);
	if (document.body.clientHeight + dt - t - h >= ch) iY = (p=="image")? t + h : t + h + 6;
    else iY  = (t - dt < ch) ? ((p=="image")? t + h : t + h + 6) : t - ch;
    if (dw + dl - l >= cw) iX = l; else iX = (dw >= cw) ? dw - cw + dl : dl;
